...
public class MasterController {
	...
	public @ResponseBody Info updateBillStatusToProduction(@RequestParam("spOrderNo") List<Integer> spOrderNo,
			@RequestParam("billStatus") int billStatus) {
		int res = 0;
	...
	public @ResponseBody Info updateItemHsnAndPer(@RequestParam("items") List<Integer> items,
			@RequestParam("itemHsncd") String itemHsncd, @RequestParam("itemTax1") float itemTax1,
	...
	public @ResponseBody Info updateItemRateAndMrp(@RequestParam("id") int id, @RequestParam("rate") float rate,
			@RequestParam("mrp") float mrp) {
		Info info = null;
		try {
	...
	public List<GetRegSpCakeOrders> getRegSpCakeOrderHistory(@RequestParam String spDeliveryDt, @RequestParam int frId,
			@RequestParam List<String> catId) {
	...
	public @ResponseBody Info saveFrMenuConfigure(@RequestParam List<String> frIdList,
		 ...
		 * List<PostFrItemStockHeader> postFrItemStockHeaderList = new
		 * ArrayList<PostFrItemStockHeader>();
		 * 
		 * postFrItemStockHeaders =
		 * postFrOpStockHeaderRepository.save(postFrItemStockHeader);
		 * postFrItemStockHeaderList.add(postFrItemStockHeaders); int headerId =
		 * postFrItemStockHeader.getOpeningStockHeaderId();
		 * 
		 * List<PostFrItemStockDetail> PostFrItemStockDetailList =
		 * postFrItemStockHeader.getPostFrItemStockDetailList();
		 * 
		 * for (int j = 0; j < PostFrItemStockDetailList.size(); j++) {
		 * 
		 * PostFrItemStockDetail postFrItemStockDetail =
		 * PostFrItemStockDetailList.get(j);
		 * 
	...
	public @ResponseBody List<ItemForMOrder> getItemListForMOrder(@RequestParam("itemGrp1") int itemGrp1,
			...
			@RequestParam("isDairyMart") int isDairyMart) {
		...
		try {
			if (isDairyMart == 0) {
				if (ordertype == 0) {
					System.err.println(
							"itemGrp1" + itemGrp1 + "frId" + menuId + "ordertype" + ordertype + "prodDate" + prodDate);
	...
	public Info updateFlavourConf(@RequestParam("flavId") int flavId, @RequestParam("rate") float rate,
			@RequestParam("mrp") float mrp) {
		Info info = new Info();
		try {
	...
	// Anmol
	@RequestMapping(value = { "/saveItemDepartment" }, method = RequestMethod.POST)
	public @ResponseBody ItemDepartment saveItemDepartment(@RequestBody ItemDepartment itemDepartment) {

		ItemDepartment res = null;
		try {
			res = itemDepartmentRepo.save(itemDepartment);
		} catch (Exception e) {
			res = new ItemDepartment();
		}
		return res;
	}

	@RequestMapping(value = "/deleteItemDepartment", method = RequestMethod.POST)
	public @ResponseBody Info deleteItemDepartment(@RequestParam int deptId, @RequestParam int status) {

		Info info = new Info();
		int isDelete = itemDepartmentRepo.deleteByDeptId(deptId, status);
	...
	@RequestMapping(value = "/activeItemDepartment", method = RequestMethod.POST)
	public @ResponseBody Info activeItemDepartment(@RequestParam int deptId, @RequestParam int status) {

		Info info = new Info();
		int isDelete = itemDepartmentRepo.activeByDeptId(deptId, status);
	...
	@RequestMapping(value = "/getItemDepartmentByDeptId", method = RequestMethod.POST)
	public @ResponseBody ItemDepartment getItemDepartmentByDeptId(@RequestParam("deptId") int deptId) {

		ItemDepartment dept = new ItemDepartment();
		try {
			dept = itemDepartmentRepo.findByDeptId(deptId);
		} catch (Exception e) {
	...
	@RequestMapping(value = "/getAllItemDepartment", method = RequestMethod.GET)
	public @ResponseBody List<ItemDepartment> getAllItemDepartment() {

		List<ItemDepartment> res;
		try {
			res = itemDepartmentRepo.findAllByIsActiveAndDelStatus(0, 0);
		} catch (Exception e) {
			res = new ArrayList<>();
			e.printStackTrace();
		}
