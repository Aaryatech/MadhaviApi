...
import com.ats.webapi.repository.prod.ProdItemDeptWiseModelRepo;
...
public class TempProdApi {
	...
	/*
	 * @Autowired GetItemwiseProdPlanRepo getItemwiseProdPlanRepo;
	 * 
	 * @Autowired GetItemDetailForMixingRepo getItemDetailForMixingRepo;
	 * 
	 * @Autowired GetSfDetailForMixingRepo getSfDetailForMixingRepo;
	 * 
	 * 
	 * @Autowired GetSFDataForMixingRepo getSFDataForMixingRepo;
	 */
	@Autowired
	...
	public @ResponseBody GetTempMixItemDetailList getTempMixItemDetailRepo(
			@RequestParam("prodHeaderId") int prodHeaderId) {
	...
	public @ResponseBody ProdMixingReqP1List getSfPlanDetailForMixing(@RequestParam("headerId") int headerId) {
		...
		try {
			...
			// List<GetSFPlanDetailForMixing>
			// sfPlanDetailForMixing=getSFPlanDetailForMixingRepo.getSFAndPlanDetailForMixing(headerId);
	...
	public @ResponseBody GetSFPlanDetailForMixingList getSfPlanDetailForBom(@RequestParam("headerId") int headerId) {
		...
		try {
			...
			List<GetSFPlanDetailForMixing> sfPlanDetailForBom = getSFPlanDetailForMixingRepo
					.getSfPlanDetailForBom(headerId);
	...
	public @ResponseBody GetProdPlanHeaderList getProdPlanHeader(@RequestParam("fromDate") String fromDate,
	...
	/*
	 * @RequestMapping(value = { "/getItemwisePlanDetail" }, method =
	 * RequestMethod.GET) public @ResponseBody GetItemwisePlanDetailList
	 * getItemwisePlanDetail() {
	 * 
	 ...
	 * try {
	 * 
	 * List<GetItemwisePlanDetail>
	 * itemWisePlan=getItemwiseProdPlanRepo.getItemQty();
	 * 
	 ...
	 * 
	 * } else { info.setError(true); info.setMessage("failed :"); }
	 * 
	 ...
	 * 
	 * 
	 * @RequestMapping(value = { "/getItemDetailForMixing" }, method =
	 * RequestMethod.POST) public @ResponseBody GetItemDetailForMixingList
	 * getItemDetailForMixing(@RequestParam("itemIdList") List<String> itemIdList) {
	 * 
	 ...
	 * try {
	 * 
	 * List<GetItemDetailForMixing>
	 * itemForMixing=getItemDetailForMixingRepo.getMixingItem(itemIdList);
	 * 
	 ...
	 * } else { info.setError(true); info.setMessage("failed :"); }
	 * 
	 * itemMixList.setItemDetailForMixing(itemForMixing); itemMixList.setInfo(info);
	 * 
	 * 
	 * }catch (Exception e) {
	 ...
	 * 
	 * 
	 * @RequestMapping(value = { "/getSfDetailForMixing" }, method =
	 * RequestMethod.POST) public @ResponseBody GetSfDetailForMixingList
	 * getSfDetailForMixing(@RequestParam("sfIdList") List<String> sfIdList) {
	 * 
	 ...
	 * try {
	 * 
	 * List<GetSfDetailForMixing> sfDetailLits=
	 * getSfDetailForMixingRepo.getSfDetailForMixing(sfIdList);
	 * 
	 ...
	 * } else { info.setError(true); info.setMessage("failed :"); }
	 * 
	 * sfDetailMix.setSfDetailForMixing(sfDetailLits); sfDetailMix.setInfo(info);
	 * 
	 * 
	 * }catch (Exception e) {
	 ...
	 */
	/*
	 * @RequestMapping(value = { "/getSFDataForMixing" }, method =
	 * RequestMethod.POST) public @ResponseBody GetSFDataForMixingList
	 * getSFData(@RequestParam("qty")int qty,@RequestParam("sfId")int sfId) {
	 * 
	 ...
	 * try {
	 * 
	 * List<GetSFDataForMixing>
	 * sFDataForMixing=getSFDataForMixingRepo.getSFData(qty, sfId);
	 * 
	 ...
	 * } else {
	 * 
	 * info.setError(true); info.setMessage("failed"); }
	 * 
	 ...
	 * 
	 * }catch (Exception e) {
	 * System.out.println("Error getting sf Data For Mixing "); e.printStackTrace();
	 * 
	 * }
	 * 
	...
	public @ResponseBody GetProdDetailBySubCatList getProdDetailBySubCat(
			@RequestParam("prodHeaderId") int prodHeaderId) {
		...
		try {
			...
			List<GetProdDetailBySubCat> prodDetailBySubCat = getProdDetailBySubCatRepo
					.getProdDetailBySubCat(prodHeaderId);
	...
	@Autowired
	ProdItemDeptWiseModelRepo prodItemDeptWiseModelRepo;

	@RequestMapping(value = { "/getDeptWiseItemListForProduction" }, method = RequestMethod.POST)
	public @ResponseBody List<ProdItemDeptWiseModel> getDeptWiseItemListForProduction(
			@RequestParam("headerIds") List<Integer> headerIds) {
		
		System.err.println("HEADER IDS - "+headerIds);

		List<ProdItemDeptWiseModel> prodDetailList = new ArrayList<>();

		try {
			prodDetailList = prodItemDeptWiseModelRepo.getDeptWiseItemsForProd(headerIds);
			System.err.println("RESULT - "+prodDetailList);
