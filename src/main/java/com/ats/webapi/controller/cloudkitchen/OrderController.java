...
//	}
						...
						for (GetOrderDisplay order : res) {
							...
							if (orderIds.get(i) == order.getOrderId()) {
								...
								GetOrderHeaderDisplay header = new GetOrderHeaderDisplay(order.getOrderId(),
										...
										order.getCityName(), order.getAreaName(),order.getPincode(), detailList);
						...
						for (GetOrderDisplay order : res) {
							...
							if (orderList.get(i).getOrderId() == order.getOrderId()) {
								...
								GetOrderDetailDisplay detail = new GetOrderDetailDisplay(order.getOrderDetailId(),
										...
										order.getMrp(), order.getRate(), order.getTaxableAmt(), order.getCgstPer(),
										order.getSgstPer(), order.getIgstPer(), order.getCgstAmt(), order.getSgstAmt(),
										order.getIgstAmt(), order.getDiscAmt(), order.getTaxAmt(), order.getTotalAmt(),
										...
										order.getItemUom(), order.getUomId());
	...
	public @ResponseBody List<GetOrderHeaderDisplay> getOrdersByFrAndStatus(@RequestParam("frId") int frId, @RequestParam("status") List<Integer> status) {
