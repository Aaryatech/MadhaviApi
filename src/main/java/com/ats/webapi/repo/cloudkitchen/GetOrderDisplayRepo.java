...
public interface GetOrderDisplayRepo extends JpaRepository<GetOrderDisplay, Integer> {
	...
	@Query(value="select * from (SELECT\r\n" + 
			...
			"        sup.uom_id,\r\n" + 
			"        ct.city_name,\r\n" + 
			"        '' as area_name,  COALESCE((SELECT a.pincode FROM mn_cust_address a WHERE a.cust_address_id=h.address_id),'') as pincode, c.phone_number as cust_phone,c.whatsapp_no as cust_whats_app \r\n" + 
			"    FROM\r\n" + 
			"        tn_order_header h,\r\n" + 
			"        tn_order_detail d,\r\n" + 
			"        m_customer c,\r\n" + 
			...
			" LEFT JOIN(\r\n" + 
			"    SELECT t.trail_id,t.order_id,t.status,t.ex_var1 AS trail_remark FROM tn_order_trail t WHERE t.trail_id IN ( SELECT MAX(trail_id) FROM tn_order_trail GROUP BY order_id ) " + 
			") t4\r\n" + 
			"ON\r\n" + 
			"    t1.order_id = t4.order_id AND t1.order_status=t4.status " +
			" ) tb order by tb.order_status",nativeQuery=true)
	...
	@Query(value="SELECT * FROM (SELECT\r\n" + 
			...
			") t4\r\n" + 
			"ON\r\n" + 
			"    t1.order_id = t4.order_id AND t1.order_status=t4.status) tb ORDER BY tb.order_status,tb.delivery_date, tb.delivery_time ",nativeQuery=true)
	...
	@Query(value="SELECT * FROM (SELECT\r\n" + 
			...
			"        mn_city ct \r\n" + 
			"    WHERE\r\n" + 
			"        h.del_status = 0 AND d.del_status = 0 AND h.order_id=d.order_id AND c.del_status = 0 AND h.cust_id = c.cust_id AND d.item_id = i.id AND d.item_id = sup.item_id AND h.city_id = ct.city_id  AND h.fr_id = :frId AND h.order_status IN(:status) AND h.delivery_date BETWEEN :fromDate AND :toDate  \r\n" + 
			"    ORDER BY\r\n" + 
			"        h.order_status\r\n" + 
			") t1\r\n" + 
			"LEFT JOIN(\r\n" + 
	...
	@Query(value="SELECT\r\n" + 
			...
			"        COALESCE(t3.fr_emp_name, '') AS order_delivered_by_name,\r\n" + 
			"        DATE_FORMAT(t1.delivery_date, '%d-%m-%Y') AS delivery_date_display,\r\n" + 
			"        TIME_FORMAT(t1.delivery_time, '%h:%i %p') AS delivery_time_display,\r\n" + 
			"        COALESCE(t4.trail_remark, '') AS trail_remark\r\n" + 
			"    FROM\r\n" + 
