...
public interface SubCatBillRepRepo extends JpaRepository<SubCatBillRep, Integer> {
	...
	@Query(value = "SELECT\n" + 
			...
			"        m_item i\n" + 
			"    WHERE\n" + 
			"        tb.del_status = 0 AND tb.bill_no = td.bill_no AND tb.fr_id =:frId AND tb.bill_date BETWEEN :fromDate  AND :toDate AND td.cat_id = c.cat_id AND i.id = td.item_id AND i.item_grp2 = sc.sub_cat_id AND td.cat_id != 5\n" + 
			"    GROUP BY\n" + 
			"        i.item_grp2\n" + 
			"    UNION ALL\n" + 
			"SELECT\n" + 
			...
			"    m_sp_cake s\n" + 
			"WHERE\n" + 
			"    tb.del_status = 0 AND tb.bill_no = td.bill_no AND tb.fr_id =:frId AND tb.bill_date BETWEEN :fromDate  AND :toDate AND td.cat_id = c.cat_id AND s.sp_id = td.item_id AND 5 = sc.cat_id AND c.cat_id = 5\n" + 
			"GROUP BY\n" + 
			"    sc.cat_id\n" + 
			") t\n" + 
			"ORDER BY\n" + 
			"    cat_id", nativeQuery = true)
...
//		"        WHERE tb.del_status=0  AND tb.bill_no=td.bill_no And tb.fr_id=:frId AND tb.bill_date BETWEEN  :fromDate  AND :toDate  AND td.cat_id=c.cat_id AND s.sp_id=td.item_id AND 5=sc.cat_id and c.cat_id=5 GROUP BY sc.cat_id 
