...
public interface FrItemWiseSellDataRepo extends JpaRepository<FrItemWiseSellData, Integer> {
	...
	@Query(value = "SELECT\r\n" + 
			...
			"            m_cat_sub subcat\r\n" + 
			"        WHERE\r\n" + 
			"            sd.sell_bill_no = sh.sell_bill_no AND i.id = sd.item_id AND subcat.sub_cat_id = i.item_grp2 AND sd.cat_id != 5 AND sh.del_status = 0 AND sh.bill_date BETWEEN :fromDate AND :toDate AND sh.fr_id = :frId AND sd.cat_id IN(:catIdList) AND sh.ext_int2>0\r\n" + 
			"        GROUP BY\r\n" + 
			"            i.id,\r\n" + 
			"            sh.fr_id,\r\n" + 
			"            i.item_grp2,\r\n" + 
			...
			"        m_cat_sub catsub\r\n" + 
			"    WHERE\r\n" + 
			"        sd.sell_bill_no = sh.sell_bill_no AND sp.sp_id = sd.item_id AND sd.cat_id = catsub.cat_id AND sd.cat_id = 5 AND sh.del_status = 0 AND sh.bill_date BETWEEN :fromDate AND :toDate AND sh.fr_id = :frId AND sd.cat_id IN(:catIdList) AND sh.ext_int2>0\r\n" + 
			"    GROUP BY\r\n" + 
			"        sp.sp_id,\r\n" + 
			"        sh.fr_id,\r\n" + 
			"        catsub.sub_cat_id,\r\n" + 
			...
			"    item_name" + 
			" ", nativeQuery = true)
	List<FrItemWiseSellData> getItemwiseSellBillDataForOnline(@Param("fromDate") String fromDate, @Param("toDate") String toDate, @Param("catIdList") List<Integer> catIdList, @Param("frId") int frId);
	...
	@Query(value = "SELECT\r\n" + 
			...
			"    item_name" + 
			" ", nativeQuery = true)
	List<FrItemWiseSellData> getItemwiseSellBillDataForPOS(@Param("fromDate") String fromDate, @Param("toDate") String toDate, @Param("catIdList") List<Integer> catIdList, @Param("frId") int frId);
