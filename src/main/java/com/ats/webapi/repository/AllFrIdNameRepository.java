...
public interface AllFrIdNameRepository extends JpaRepository<AllFrIdName, Integer> {
	...
	@Query(value=" SELECT m_franchisee.fr_id,CONCAT(m_franchisee.fr_name,' - ',m_franchisee.fr_code) as fr_name from m_franchisee,m_fr_route,m_franchise_sup WHERE m_fr_route.route_id=m_franchisee.fr_route_id and m_franchise_sup.fr_id=m_franchisee.fr_id and m_franchisee.del_status=0 and m_fr_route.del_status=0 order by m_franchisee.fr_name Asc",nativeQuery=true)
	...
	public List<AllFrIdName> findNonOrders(@Param("orderDate") String orderDate, @Param("menuId") int menuId);
		...
		@Query(value=" SELECT fr_id,CONCAT(m_franchisee.fr_name,' - ',m_franchisee.fr_code) as fr_name from m_franchisee where fr_id=:frId",nativeQuery=true)
