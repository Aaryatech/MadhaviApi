...
public interface GetBillHeaderRepository extends JpaRepository<GetBillHeader, Integer> {
	...
	@Query(value=" SELECT\n" + 
			...
			"        m_franchisee " + 
			"    WHERE " + 
			"        t_sell_bill_header.bill_date BETWEEN :fromDate AND :toDate AND t_sell_bill_header.fr_id = m_franchisee.fr_id AND t_sell_bill_header.del_status = 0 AND IF(:configType=0 , t_sell_bill_header.ext_int2>=0, IF(:configType=1 , t_sell_bill_header.ext_int2=0, t_sell_bill_header.ext_int2>0)) AND(\n" + 
			"        SELECT " + 
			"            m_franchisee.kg_1 " + 
			"        FROM " + 
			"            m_franchisee\n" + 
	...
	List<GetBillHeader> getBillHeaderForAllFrAllNew(@Param("fromDate")String fromDate, @Param("toDate")String toDate,@Param("temp") List<Integer> temp,@Param("configType") int configType);
	...
	@Query(value=" SELECT t_sell_bill_header.sell_bill_no as bill_no ,t_sell_bill_header.invoice_no, t_sell_bill_header.bill_date "
			...
			+ " CONCAT(m_franchisee.fr_name,' - ',m_franchisee.fr_code) as fr_name,t_sell_bill_header.user_name as party_name,t_sell_bill_header.user_gst_no as party_gstin,m_franchisee.fr_address as party_address ,'0' AS eway_bill_no FROM t_sell_bill_header,m_franchisee WHERE "
			+ " t_sell_bill_header.bill_date BETWEEN :fromDate AND :toDate "
			+ " AND t_sell_bill_header.fr_id=m_franchisee.fr_id AND t_sell_bill_header.del_status=0 AND (Select m_franchisee.kg_1 from m_franchisee where t_sell_bill_header.fr_id=m_franchisee.fr_id)=1 AND IF(:configType=0 , t_sell_bill_header.ext_int2>=0, IF(:configType=1 , t_sell_bill_header.ext_int2=0, t_sell_bill_header.ext_int2>0))" + 
			"",nativeQuery=true)
	List<GetBillHeader> getBillHeaderForAllFr3New(@Param("fromDate")String fromDate, @Param("toDate")String toDate, @Param("configType") int configType);
