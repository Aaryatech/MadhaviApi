...
public interface ItemWiseDetailRepository extends JpaRepository<ItemWiseDetail, Long>{
	...
	@Query(value="SELECT t_bill_detail.bill_detail_no, t_bill_header.invoice_no as bill_no ,t_bill_header.bill_date, t_bill_detail.item_id ,  m_item.item_name ,t_bill_detail.rate,COALESCE(SUM(bill_qty),0) AS qty, COALESCE(SUM(t_bill_detail.grand_total),0) AS total ,t_bill_detail.grn_type,t_bill_detail.expiry_date, COALESCE(SUM(t_bill_detail.remark),0) AS disc_amt FROM t_bill_detail , m_item, t_bill_header WHERE  m_item.id= t_bill_detail.item_id AND t_bill_detail.bill_no IN(SELECT bill_no FROM t_bill_header WHERE bill_date BETWEEN :fromDate AND :toDate AND fr_id=:frId) AND t_bill_detail.bill_no=t_bill_header.bill_no AND t_bill_detail.cat_id=:catId and t_bill_header.del_status=0 and t_bill_detail.del_status=0 GROUP BY t_bill_detail.item_id, t_bill_detail.bill_no ORDER BY t_bill_header.invoice_no DESC",nativeQuery=true)
	...
	@Query(value="SELECT t_bill_detail.bill_detail_no, t_bill_header.invoice_no as bill_no ,t_bill_header.bill_date, t_bill_detail.item_id ,  "
			+ "m_sp_cake.sp_name AS item_name ,t_bill_detail.rate,COALESCE(SUM(bill_qty),0) AS qty, COALESCE(SUM(t_bill_detail.grand_total),0) AS total ,"
			+ "t_bill_detail.grn_type,t_bill_detail.expiry_date, COALESCE(SUM(t_bill_detail.remark),0) AS disc_amt FROM t_bill_detail , m_sp_cake, "
			+ "t_bill_header WHERE  m_sp_cake.sp_id= t_bill_detail.item_id AND t_bill_detail.bill_no IN(SELECT bill_no FROM t_bill_header "
			+ "WHERE bill_date BETWEEN :fromDate AND :toDate AND fr_id=:frId) AND t_bill_detail.bill_no=t_bill_header.bill_no AND "
			+ "t_bill_detail.cat_id=:catId and t_bill_header.del_status=0 and t_bill_detail.del_status=0 "
			+ "GROUP BY t_bill_detail.item_id, t_bill_detail.bill_no ORDER BY  m_item.item_name, t_bill_header.bill_date DESC",nativeQuery=true)
	...
	@Query(value="SELECT\n" + 
			...
			"        0) AS total ,\n" + 
			"        t_bill_detail.grn_type,t_bill_detail.expiry_date, COALESCE(SUM(t_bill_detail.remark),0) AS disc_amt \n" + 
			"    FROM\n" + 
			"        t_bill_detail ,\n" + 
			"        m_item,\n" + 
			"        t_bill_header \n" + 
			...
			"    GROUP BY\n" + 
			"        t_bill_detail.item_id,\n" + 
			"        t_bill_detail.bill_no ORDER BY  m_item.item_name, t_bill_header.bill_date DESC",nativeQuery=true)
	...
	@Query(value="SELECT\n" + 
			...
			"        COALESCE(SUM(t_bill_detail.remark),0) AS disc_amt \n" + 
			"    FROM\n" + 
			"        t_bill_detail ,\n" + 
			"        m_item,\n" + 
			"        t_bill_header \n" + 
	...
	@Query(value="SELECT\n" + 
			...
			"        t_bill_detail.item_id,\n" + 
			"        t_bill_detail.bill_no ORDER BY  m_item.item_name, t_bill_header.bill_date DESC",nativeQuery=true)
	List<ItemWiseDetail> findAllSpecialCakeWiseDetailReport(@Param("frId") int frId, @Param("fromDate") String fromDate, @Param("toDate") String toDate);
