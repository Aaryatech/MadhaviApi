...
public interface StockSellRepository  extends JpaRepository<StockRegSpSell ,Integer>{
	...
	@Query(value = "\n" + 
			...
			"(select item_id,rm_id,rm_qty,no_pieces_per_item from m_item_detail where item_id=:itemId and del_status=0) a\n" + 
			"LEFT JOIN \n" + 
			"(SELECT t_sell_bill_detail.item_id,COALESCE(SUM(CASE WHEN bill_stock_type = 1 THEN qty ELSE 0 END),0) as sell_qty FROM t_sell_bill_detail WHERE t_sell_bill_detail.item_id in(:itemId) AND \n" + 
